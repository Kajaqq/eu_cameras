...
def get_video_frame(video_file: Path):
    ...
    ffmpeg.output(scaled, filename=output_path, vframes=1).run(overwrite_output=True, quiet=True)
    video_file.unlink()
...
def main(file_path=None):
    ...
    video_files = [f for f in file_path.glob('*.mp4')]

    if video_files:
        with ProcessPoolExecutor() as executor:
            list(executor.map(get_video_frame, video_files))

    image_files = [f for f in file_path.iterdir() if f.suffix.lower() in ['.jpg', '.png']]
